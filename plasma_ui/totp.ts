...
export class TOTP {
    ...
    static async generate(key: string, options?: Options): Promise<{ otp: string; expires: number }> {
        ...
        const hmacKey = await crypto.subtle.importKey("raw", keyBuffer, { name: "HMAC", hash: "SHA-1" }, false, ["sign"])
        const signature = await crypto.subtle.sign("HMAC", hmacKey, this.hex2buf(timeHex))
    ...
    //  * @type {SubtleCrypto}
    //  */
    // private static readonly crypto: SubtleCrypto = crypto.subtle
